...
pub trait Handleable: Send + Sync + Debug {
    ...
    fn prepare_response(
        ...
        Response::builder().status(http_status_code).body(body)
    }
}
...
impl Router {
    ...
    pub async fn handle_request(
        ...
        let response = route.handle_data(
            route_req_params,
            str::from_utf8(&b).unwrap().to_string(),
            supervisor_arc,
        );

        response.await.unwrap_or_else(|err| {
            let err_response_text = err.to_string();
            let err_bytes = bytes::Bytes::from(err_response_text);
            let err_body = Full::new(err_bytes);
    ...
    fn route(&self, req_method: &str, req_path: &str) -> &Box<(dyn Handleable + 'static)> {
        ...
        let route = self.routes.iter().find(|route| {
            ...
            route_segments.iter().zip(req_path_segments.iter()).all(
